//! Handlebars-based templating for the JoltR framework: registry construction,
//! template loading, and render helpers. Modules land in subsequent PRD items.
//!
//! [`TemplateEngine`] (JOLTR-RS-106) wraps a [`handlebars::Handlebars`] registry
//! and is constructed from a filesystem directory of `.hbs` templates via
//! [`TemplateEngine::new`]. The render API (JOLTR-RS-107), the custom helper
//! registration (JOLTR-RS-108), and the closing test bundle (JOLTR-RS-109) build
//! on the registry surface this slice exposes.
//!
//! Architectural decisions pinned here for JOLTR-RS-107..109 to build on:
//!
...
//!    outer state; the JoltR framework does not need that flexibility (helpers
...
//!    knob — every JoltR deployment uses `.hbs` — but the registry built here
...
//!    [`JwtDecodeError`](../../joltr_utils/jwt/enum.JwtDecodeError.html) (072)
//!    and [`MigrationError`](../../joltr_db/enum.MigrationError.html) (100)
...
//!    the call site. JOLTR-RS-094's `read_migration_files` (joltr-db) takes
...
//!    so the JOLTR-RS-109 closing-test bundle can pin specific failure
...
//!    framework default — every JoltR deployment gets the same surface so
...
//!    the only NEW helper JoltR registers is `json` — a
...
impl TemplateEngine {
    ...
    /// of the directory-loaded set; the render API (JOLTR-RS-107) and helper
    /// registration (JOLTR-RS-108) consume this same registry internally.
    pub fn registry(&self) -> &Handlebars<'static> {
        &self.registry
    }

    /// Mutable companion to [`Self::registry`]; needed for the helper
    /// registration that lands in JOLTR-RS-108.
...
// Register JoltR's framework-default helper set on a [`Handlebars`] registry
...
mod tests {
    ...
    /// the `TestDir` fixture used by `joltr-db`'s `read_migration_files`
    /// tests (JOLTR-RS-094) — keeps the convention consistent across crates.
    struct TestDir {
    ...
    /// PRD-mandated verification for JOLTR-RS-106: handlebars registry created
    ...
    /// helper registration in JOLTR-RS-108 will register helpers through this
    ...
    /// PRD-mandated verification for JOLTR-RS-107: render `hello.hbs` (which
    ...
    /// strict-mode semantics — the JOLTR-RS-109 closing-test bundle and
    ...
    /// PRD-mandated verification for JOLTR-RS-108: `{{#if (eq status "active")}}`
    ...
    /// The `json` helper (the only NEW helper added by JOLTR-RS-108 — the
