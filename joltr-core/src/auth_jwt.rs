//! JWT validation `tower::Layer` (JOLTR-RS-072..074).
//!
//! [`AuthJwtLayer`] (JOLTR-RS-072) consumes the
//! [`BearerToken`](crate::auth_bearer::BearerToken) stashed by
//! [`AuthBearerLayer`](crate::AuthBearerLayer) (JOLTR-RS-071), calls
//! [`joltr_utils::jwt::decode`] with the configured secret + algorithm, and
...
//!    it. This decision was pinned by the JOLTR-RS-071 progress note's "(b)
...
//!    and throwing away the claims would force JOLTR-RS-073 to either re-call
//!    [`joltr_utils::jwt::decode`] or expose half-validated state. The
//!    JOLTR-RS-073 slice closes the contract by pinning the extension-key
...
/// parsed [`JwtClaims`] into request extensions (JOLTR-RS-072). See module docs
...
/// validates the JWT via [`joltr_utils::jwt::decode`], and either stashes a
    ...
    S::Error: Send + 'static,
{
    ...
    fn call(&mut self, mut req: AxumRequest) -> Self::Future {
        ...
        // `AuthBearerService::call` (JOLTR-RS-071), `ParseBodyService::call`
        // (JOLTR-RS-059), and `CorsService::call` (JOLTR-RS-056).
