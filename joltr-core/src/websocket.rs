...
///
/// This is a JoltR-owned newtype rather than a re-export of
...
/// normal closure, `1011` internal error, etc.). A higher-level JoltR close
...
impl From<AxumMessage> for WsMessage {
    /// Converts an inbound axum frame into the JoltR-side [`WsMessage`].
...
///
/// ```ignore
/// use joltr_core::{WebSocketHandler, WebSocketSender, WsMessage};
///
/// struct EchoHandler;
///
...
pub trait WebSocketHandler {
    ...
    /// and decoded during the pre-upgrade auth flow (JOLTR-RS-076/077).
...
mod tests {
    ...
    //! - JOLTR-RS-119 tests (preserved): five witnesses for [`WebSocketSender`]
    ...
    // JOLTR-RS-131 tests: subscribe + forward + unsubscribe on WebSocketSender.
    ...
    /// JOLTR-RS-132: publish more messages than the broadcast channel capacity
    ...
    /// JOLTR-RS-133: on_close drops the Subscription handle, which aborts the
