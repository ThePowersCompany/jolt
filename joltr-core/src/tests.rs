...
//!
//! Submodule names are deliberate: `cargo test -p joltr-core -- tests::method`
//! and `cargo test -p joltr-core -- tests::status_code` filter cleanly to the
//! relevant slice.
...
mod status_code {
    ...
    fn into_axum_status_matches_constants_for_each_named_variant() {
        ...
        for (joltr, axum_status) in cases {
            let converted: axum::http::StatusCode = (*joltr).into();
...
mod response {
    ...
    async fn into_axum_response_serializes_json_with_content_type_and_status() {
        ...
        let joltr_response = Response::new(StatusCode::Ok, json!({"hello": "world"}));
        let axum_response: axum::response::Response = joltr_response.into();
    ...
    async fn into_axum_response_forwards_custom_headers() {
        ...
        let mut joltr_response = Response::new(StatusCode::Created, 7u32);
        joltr_response.headers.insert(
    ...
    async fn into_axum_response_with_string_body_uses_text_plain() {
        ...
        let joltr_response = Response::new(StatusCode::Ok, "hello".to_string());
        let axum_response: axum::response::Response = joltr_response.into();
    ...
    async fn into_axum_response_with_str_body_uses_text_plain() {
        ...
        let joltr_response = Response::new(StatusCode::Ok, "world");
        let axum_response: axum::response::Response = joltr_response.into();
    ...
    async fn into_axum_response_with_string_body_forwards_custom_headers() {
        ...
        let mut joltr_response = Response::new(StatusCode::Created, "ok".to_string());
        joltr_response.headers.insert(
            HeaderName::from_static("x-trace-id"),
    ...
    async fn into_axum_response_overrides_caller_set_content_type() {
        ...
        let mut joltr_response = Response::new(StatusCode::Ok, 1u32);
        joltr_response
            .headers
            .insert(CONTENT_TYPE, HeaderValue::from_static("application/xml"));
        let response: axum::response::Response = joltr_response.into();
...
mod request_ext {
    ...
    fn take_response_returns_none_when_no_response_stashed() {
        // JOLTR-RS-035: the stash defaults to empty so a finished-without-stash
...
mod endpoint {
    //! PRD JOLTR-RS-028 verification ("Trait compiles") plus the load-bearing
    //! contracts the registry layer (JOLTR-RS-029) and Router layer
    //! (JOLTR-RS-033..) will lean on: object-safety with `Send + Sync`, and a
    ...
    fn trait_is_object_safe_with_send_sync_bounds() {
        // Locks in JOLTR-RS-029's prerequisite: the registry stores
...
mod endpoint_registry {
    //! PRD JOLTR-RS-029 verification ("register two endpoints, registry has
    //! length 2") plus the empty-registry contract that JOLTR-RS-031's
    ...
    fn register_two_endpoints_yields_length_two() {
        // PRD-mandated verification for JOLTR-RS-029.
    ...
    fn default_matches_new() {
        // Default impl is what `JoltRServer` will use to embed an empty
    ...
    fn sort_orders_paths_by_length_descending() {
        // PRD-mandated verification for JOLTR-RS-030: registering ["/", "/api/hello", "/api"]
        // and calling sort yields ["/api/hello", "/api", "/"]. Longest-first matters
        // because JOLTR-RS-031's `build_router` will pick the first matching route, so
    ...
    fn sort_on_empty_registry_is_a_noop() {
        // Guards the empty-registry contract that JOLTR-RS-031's build_router
    ...
    // --- duplicate path/method detection (JOLTR-RS-032) ---
    //
    ...
    // Endpoint design (JOLTR-RS-028), which uses exact string matching via
    ...
    /// JOLTR-RS-031 verification: build_router from two endpoints produces an
    ...
    mod build_router {
        ...
        async fn two_endpoints_each_dispatch_correctly() {
            // PRD-mandated verification for JOLTR-RS-031: two endpoints, two
        ...
        async fn handler_sees_method_path_query_body_and_cookie() {
            // Locks in the cross-phase glue: the axum→JoltR request conversion
            // populates method/path/query_params/body/cookies before invoking
            // the handler. If a future refactor of build_joltr_request drops
...
mod router {
    //! PRD JOLTR-RS-033 verification ("Service impl compiles") plus a behavioral
    ...
    //! contract is what JOLTR-RS-035's finished-flag short-circuit reads from;
    ...
    async fn service_call_preserves_caller_supplied_request_ext() {
        // JOLTR-RS-035 inverted the JOLTR-RS-033 contract: Router now PRESERVES a
    ...
    mod registry_dispatch {
        //! PRD JOLTR-RS-034 verification: `Router::call` parses path + method
        //! from the inbound request, walks the (sorted) endpoint registry, and
        //! either dispatches to a matching handler or returns 404.
        //!
        //! 405 method-mismatch differentiation is JOLTR-RS-037's territory; per
        ...
        async fn method_mismatch_on_registered_path_returns_405() {
            // PRD JOLTR-RS-037 verification: a registered path hit with the
            ...
            // exists, method not supported," and JoltR's router now honors that
            // distinction.
            let mut registry = EndpointRegistry::new();
            registry.register(EchoEndpoint {
                path: "/hello",
        ...
        async fn unparseable_http_verb_returns_404_even_for_registered_path() {
            // Pins the conservative-404 contract for HTTP verbs JoltR's
            // `Method` enum doesn't recognize (e.g. CONNECT, TRACE). Returning
            // 405 here would require enumerating JoltR's vocabulary to a
            // caller that already sent something JoltR doesn't speak; 404
        ...
        async fn finished_flag_with_stashed_response_short_circuits_handler() {
            // PRD-mandated verification for JOLTR-RS-035: an "auth middleware"
        ...
        async fn caller_supplied_not_finished_request_ext_dispatches_normally() {
            // The third leaf of the JOLTR-RS-035 contract: a caller-supplied
        ...
        async fn finished_flag_short_circuits_handler_when_set_by_outer_tower_layer() {
            // PRD-mandated verification for JOLTR-RS-079: a real `tower::Layer`
                ...
                S::Future: Send + 'static,
            {
                ...
                fn call(&mut self, mut req: AxumRequest) -> Self::Future {
                    ...
                    // JOLTR-RS-035 preserve-existing-ext contract) and observe
        ...
        async fn full_middleware_chain_propagates_auth_rejection_back_through_outer_layer() {
            // PRD-mandated verification for JOLTR-RS-080: "Integration test:
            ...
            //      Origin` on the way back per JOLTR-RS-057's non-OPTIONS
            //      contract.
            ...
            // (JOLTR-RS-079): that test uses a FAKE FinishingLayer that flips
        ...
        async fn new_constructor_yields_ready_to_serve_router() {
            // PRD-mandated verification for JOLTR-RS-036: `Router::new(registry)`
...
mod server {
    ...
    fn new_uses_default_port_8080() {
        // PRD-mandated verification for JOLTR-RS-023: defaults include port=8080.
        let server = JoltRServer::new();
        assert_eq!(server.port, 8080);
    }

    #[test]
    fn new_starts_with_empty_registry() {
        // Locks in the default-empty contract that JOLTR-RS-026's `endpoint`
    ...
    fn endpoint_builder_registers_into_registry() {
        // PRD-mandated verification for JOLTR-RS-026: register an endpoint,
        // verify it appears in the registry.
        let server = JoltRServer::new().endpoint(StubEndpoint);
    ...
    fn builder_chain_sets_port_and_threads() {
        // PRD-mandated verification for JOLTR-RS-024: chained .port(3000).threads(4) sets values.
        let server = JoltRServer::new().port(3000).threads(4);
        assert_eq!(server.port, 3000);
    ...
    fn cors_builder_wraps_arg_in_some() {
        let server = JoltRServer::new().cors(CorsConfig::default());
        assert!(server.cors_config.is_some());
    }

    #[test]
    fn cors_config_default_is_empty_and_restrictive() {
        // PRD-mandated verification for JOLTR-RS-055: "Struct compiles with
        ...
        // no CORS access. JOLTR-RS-057 added `expose_headers`; default remains
        // restrictive.
    ...
    fn builder_methods_preserve_unspecified_defaults() {
        ...
        let default_threads = JoltRServer::new().threads;
        let server = JoltRServer::new().port(9090);
    ...
    async fn start_returns_addr_in_use_when_port_already_bound() {
        // PRD-mandated verification for JOLTR-RS-027: port binding conflict
        ...
        let result = JoltRServer::new().port(port).start(Router::new()).await;
    ...
    async fn start_binds_and_serves_404_on_empty_router() {
        // PRD-mandated verification for JOLTR-RS-025 ("server starts, curl
        ...
        let server = tokio::spawn(JoltRServer::new().port(port).start(Router::new()));
    ...
    fn build_serving_router_emits_tracing_events_for_each_request() {
        // JOLTR-RS-069: the customized TraceLayer in build_serving_router
    ...
    fn body_log_layer_emits_debug_event_for_request_body() {
        // JOLTR-RS-070: the BodyLogLayer in build_serving_router must emit
            ...
            tracing::subscriber::with_default(subscriber, || {
                ...
                rt.block_on(async {
                    ...
                    let req = axum::http::Request::builder()
                        ...
                        .body(Body::from(r#"{"message":"hello joltr"}"#))
                        .unwrap();
                    let _resp = tower::ServiceExt::oneshot(router, req).await.unwrap();
                });
            });
        ...
        assert!(
            all.contains(r#"{"message":"hello joltr"}"#),
    ...
    fn body_log_layer_suppresses_body_for_auth_paths() {
        // JOLTR-RS-070: the BodyLogLayer must NOT log body content for
...
mod cors {
    //! PRD-mandated verification for JOLTR-RS-056: "Unit test: OPTIONS /api/test
    ...
    async fn options_request_returns_204_with_cors_headers_and_marks_finished() {
        // PRD-mandated verification for JOLTR-RS-056. Builds a CorsLayer with a
        ...
        let config = CorsConfig {
            ...
            max_age: 600,
            // JOLTR-RS-057's `expose_headers` field. Empty here so the OPTIONS
        ...
        // (mirroring Router's JOLTR-RS-035 behavior) keeps THIS Arc alive in
        // extensions; the `mark_finished()` call inside the layer is observable
        // here because both sides hold clones of the same Arc.
        let ext = Arc::new(RequestExt::new());

        // The `Origin` header is required for the JOLTR-RS-058 matching helper
    ...
    async fn non_options_response_gets_allow_origin_and_expose_headers_injected() {
        // PRD-mandated verification for JOLTR-RS-057: "Unit test: GET /api/test
        ...
        let ext = Arc::new(RequestExt::new());
        // JOLTR-RS-058 matching: include `Origin` so the configured origin is
    ...
    async fn non_options_response_skips_headers_when_config_is_empty() {
        // Empty CorsConfig (the JOLTR-RS-055 restrictive default) must produce
    ...
    // -- JOLTR-RS-058: shared origin-matching helper coverage. ---------------
    //
...
mod parse_body {
    //! PRD-mandated verification for JOLTR-RS-059: "Unit test: valid JSON body
    ...
    //!    delegating, so downstream services (notably `build_joltr_request`'s
    ...
    //!    catching.
    //!
    //! Parse-failure rejection (400 + mark_finished) lands in JOLTR-RS-060's
    ...
    async fn valid_json_body_is_parsed_and_inserted_into_extensions() {
        ...
        let payload = br#"{"name":"joltr","age":7}"#;
        let req = AxumRequest::builder()
    ...
    async fn invalid_json_body_short_circuits_with_400_and_marks_finished() {
        // JOLTR-RS-060: malformed JSON must produce a 400 with an "Invalid
    ...
    async fn empty_body_is_rejected_with_400_and_marks_finished() {
        // JOLTR-RS-062: empty body → error. An empty body cannot deserialize
    ...
    async fn oversized_body_is_rejected_with_413_and_marks_finished() {
        // JOLTR-RS-062: oversized body → error. A body exceeding the
    ...
    async fn text_plain_body_is_decoded_and_inserted_into_extensions() {
        // JOLTR-RS-062: string body → success. A text/plain body must be
...
mod parse_body_string {
    //! PRD-mandated verification for JOLTR-RS-061: "Unit test: POST with
    ...
    async fn non_utf8_body_short_circuits_with_400_and_marks_finished() {
        // Mirror of JOLTR-RS-060's invalid-JSON test, adapted for the UTF-8
...
mod parse_query {
    //! PRD-mandated verification for JOLTR-RS-063: "Unit test: ?id=42 → id
    //! parsed as \"42\"."
    //!
    ...
    async fn multiple_pairs_are_each_represented_in_the_map() {
        ...
        let req = AxumRequest::builder()
            .method(HttpMethod::GET)
            .uri("/api/test?id=42&name=joltr&active=true")
            .body(Body::empty())
            .unwrap();
        ...
        assert_eq!(params.get("name").map(String::as_str), Some("joltr"));
...
mod parse_query_typed {
    //! PRD-mandated verification for JOLTR-RS-064: "Unit test: ?count=5 →
    //! `i32::from_str("5") == 5`. ?count=abc → 400."
    //!
    //! The 064 surface is a field-level extractor (`extract::<T>(params, key)`)
    //! that consumes the [`QueryParams`] map produced by JOLTR-RS-063. Tests
    //! pin:
...
mod parse_query_typed_extras {
    //! PRD-mandated verification for JOLTR-RS-065: "Unit test: ?active=true →
...
mod parse_query_typed_vec {
    //! PRD-mandated verification for JOLTR-RS-066: "Unit test: ?ids=1,2,3 →
    //! `Vec<i32> = [1,2,3]`."
    //!
...
mod auth_bearer {
    //! PRD-mandated verification for JOLTR-RS-071: "Unit test: no Authorization
    //! header → 401."
    //!
    ...
    //! re-discovering it.
    //!
    //! Module is named `auth_bearer` so `cargo test -p joltr-core -- tests::auth_bearer`
...
mod auth_jwt {
    //! PRD-mandated verification for JOLTR-RS-072: "Unit test: valid token →
    ...
    //! [`joltr_utils::jwt::decode`], and short-circuits with 401 + the
    ...
    //! stable contract.
    //!
    //! Module is named `auth_jwt` so `cargo test -p joltr-core -- tests::auth_jwt`
    ...
    async fn valid_jwt_passes_through_and_inner_runs() {
        ...
        let secret = b"joltr-rs-072-auth-jwt-test-secret";
    ...
    async fn malformed_jwt_short_circuits_with_401() {
        ...
        let layer = AuthJwtLayer::new(JwtConfig::new(
            b"joltr-rs-072-auth-jwt-test-secret".to_vec(),
            Algorithm::HS256,
        ));
        let svc = layer.layer(forbid_inner());
    ...
    async fn valid_jwt_stashes_typed_claims_via_extension_key_jwtclaims() {
        // PRD-mandated verification for JOLTR-RS-073: "valid token → JwtClaims
        // available via req.extensions().get::<JwtClaims>()". Pins the
        // extension-key contract independently of the response-body echo
        // covered by `valid_jwt_passes_through_and_inner_runs` (JOLTR-RS-072):
        ...
        let secret = b"joltr-rs-073-extension-key-contract-secret";
    ...
    async fn valid_jwt_with_custom_claims_surfaces_extra_in_extension() {
        ...
        //   * wrong algorithm  — joltr-utils' `decode_algorithm_mismatch_yields_invalid_algorithm_variant`
        ...
        // to-end through the AuthJwtLayer (rather than at the joltr-utils
        ...
        let secret = b"joltr-rs-074-auth-jwt-custom-claims-secret";
...
mod auth_websocket {
    //! PRD-mandated verification for JOLTR-RS-075: "Unit test: header
    //! 'joltr-jwt, eyJ...' → extracted token 'eyJ...'."
    //!
    //! The structural surface is wider than the single PRD-mandated case:
    //! 075's contract is the FORMAT extractor for the
    //! `Sec-WebSocket-Protocol: joltr-jwt, <token>` shape. Each rejection-path
    //! variant pinned by [`WsTokenRejectReason`] gets a dedicated test below
    //! so JOLTR-RS-076's future tower::Layer can produce a distinct 401 body
    //! per reason.
    //!
    //! Module is named `auth_websocket` so
    //! `cargo test -p joltr-core -- tests::auth_websocket` filters cleanly to
    //! this slice; matches the established `auth_bearer` / `auth_jwt`
    //! naming convention.
    use axum::http::HeaderValue;

    use crate::auth_websocket::{
        extract_jwt_token, WsJwtToken, WsTokenRejectReason, JOLTR_JWT_SUBPROTOCOL,
    };
    ...
    fn extract_jwt_token_canonical_shape_yields_token() {
        // PRD-mandated verification: header "joltr-jwt, eyJ..." → token "eyJ...".
        let header = HeaderValue::from_str(&format!("{JOLTR_JWT_SUBPROTOCOL}, {SAMPLE_JWT}"))
            .expect("static header value is valid ASCII");
        let extracted = extract_jwt_token(Some(&header))
            .expect("canonical joltr-jwt subprotocol shape must yield the token");
        assert_eq!(
    ...
    fn extract_jwt_token_extra_whitespace_around_comma_yields_token() {
        ...
        let header =
            HeaderValue::from_str(&format!("  {JOLTR_JWT_SUBPROTOCOL}   ,   {SAMPLE_JWT}   "))
                .expect("static header value is valid ASCII");
    ...
    fn extract_jwt_token_non_ascii_header_yields_not_ascii_variant() {
        ...
        let header = HeaderValue::from_bytes(b"joltr-jwt, \xC3\x28")
    ...
    #[test]
    fn extract_jwt_token_wrong_marker_yields_missing_joltr_jwt_prefix_variant() {
        // First subprotocol is not "joltr-jwt" (case-sensitive per RFC 6455
        ...
        assert_eq!(err, WsTokenRejectReason::MissingJoltRJwtPrefix);

        let header_uppercase = HeaderValue::from_str(&format!("JOLTR-JWT, {SAMPLE_JWT}")).unwrap();
        let err_uppercase = extract_jwt_token(Some(&header_uppercase)).unwrap_err();
        assert_eq!(
            err_uppercase,
            WsTokenRejectReason::MissingJoltRJwtPrefix,
    ...
    #[test]
    fn extract_jwt_token_solo_marker_yields_missing_joltr_jwt_prefix_variant() {
        // Header is exactly "joltr-jwt" with no comma + token follow-up. The
        // helper requires exactly two comma-separated parts; one part (even
        // the right marker) is rejected as a malformed shape.
        let header = HeaderValue::from_static("joltr-jwt");
        let err = extract_jwt_token(Some(&header)).unwrap_err();
        assert_eq!(err, WsTokenRejectReason::MissingJoltRJwtPrefix);
    }
    ...
    fn extract_jwt_token_empty_token_after_marker_yields_empty_token_variant() {
        // Header is "joltr-jwt," (or "joltr-jwt, ") — structurally parseable
        // (two comma-separated parts) but the token slot is empty after
        // whitespace trim. Distinct from MissingHeader and from
        // MissingJoltRJwtPrefix because BOTH parts are present, just one is
        // empty.
        let header_no_trailing = HeaderValue::from_static("joltr-jwt,");
        let err1 = extract_jwt_token(Some(&header_no_trailing)).unwrap_err();
        assert_eq!(err1, WsTokenRejectReason::EmptyToken);

        let header_trailing_space = HeaderValue::from_static("joltr-jwt, ");
    ...
    fn extract_jwt_token_more_than_two_subprotocols_yields_malformed_variant() {
        ...
        let header =
            HeaderValue::from_str(&format!("joltr-jwt, {SAMPLE_JWT}, graphql-ws")).unwrap();
        let err = extract_jwt_token(Some(&header)).unwrap_err();
    ...
    fn ws_jwt_token_newtype_exposes_borrowed_str() {
        // Pins the public surface of the WsJwtToken handle that JOLTR-RS-076
...
mod auth_ws_jwt {
    //! PRD-mandated verification for JOLTR-RS-076: "Integration test: WS
    ...
    //! that JOLTR-RS-077 will land. A real `axum::extract::ws::WebSocketUpgrade`-
    ...
    //! `cargo test -p joltr-core -- tests::auth_ws_jwt` filters cleanly to
    //! this slice; matches the established `auth_bearer` / `auth_jwt` /
    ...
    use crate::auth_websocket::{WsJwtToken, JOLTR_JWT_SUBPROTOCOL};
    use crate::request_ext::RequestExt;
    ...
    async fn invalid_token_short_circuits_with_401_and_no_upgrade() {
        ...
        let secret = b"joltr-rs-076-auth-ws-jwt-test-secret";
        // exp = 1000 → 1970; well in the past.
        let claims = JwtClaims {
            sub: Some("alice".to_owned()),
            exp: Some(1_000),
        ...
        let request_ext = Arc::new(RequestExt::new());
        let header = HeaderValue::from_str(&format!("{JOLTR_JWT_SUBPROTOCOL}, {token}"))
    ...
    async fn missing_protocol_header_short_circuits_with_401() {
        ...
        let layer = AuthWsJwtLayer::new(JwtConfig::new(
            b"joltr-rs-076-auth-ws-jwt-test-secret".to_vec(),
            Algorithm::HS256,
        ));
        let svc = layer.layer(forbid_inner());
    ...
    async fn malformed_protocol_header_short_circuits_with_401() {
        ...
        // `joltr-jwt, <token>` shape (wrong marker subprotocol) → the
        // extraction-side MissingJoltRJwtPrefix body surfaces.
        let layer = AuthWsJwtLayer::new(JwtConfig::new(
            b"joltr-rs-076-auth-ws-jwt-test-secret".to_vec(),
            Algorithm::HS256,
        ));
    ...
    async fn invalid_signature_token_short_circuits_with_401() {
        ...
        let request_ext = Arc::new(RequestExt::new());
        let header = HeaderValue::from_str(&format!("{JOLTR_JWT_SUBPROTOCOL}, {token}")).unwrap();
        let req = ws_request_with_protocol(header, Arc::clone(&request_ext));
...
mod early_termination {
    //! PRD-mandated verification for JOLTR-RS-078: "Each middleware layer
    ...
    //! `joltr_core` crate root: AuthBearerLayer, AuthJwtLayer, AuthWsJwtLayer,
    ...
    // ---------------------------------------------------------------------------
    // JOLTR-RS-081: end-to-end early-termination integration tests
    //
    ...
    // invoked. They complement the JOLTR-RS-078 layer-skip tests (above) by
    ...
    async fn multi_layer_finish_propagates_through_outer_layers() {
        ...
        // (JOLTR-RS-057 non-OPTIONS contract) into the propagated 401.
    ...
    async fn finish_after_path_match_still_skips_handler() {
        ...
        // `endpoint.handler(joltr_req).await` in Router::call). Even when
...
mod websocket {
    //! PRD-mandated verification for JOLTR-RS-121: "Write WS trait unit test:
    ...
    impl WebSocketHandler for MockLifecycleHandler {
        fn set_claims(&mut self, _claims: joltr_utils::jwt::JwtClaims) {
    ...
    async fn lifecycle_callbacks_fire_in_documented_order_on_normal_connection() {
        ...
        handler.set_claims(joltr_utils::jwt::JwtClaims {
...
mod task {
    ...
    #[tokio::test]
    async fn graceful_shutdown_stops_task_loop() {
        let run_count = Arc::new(AtomicUsize::new(0));
        ...
        scheduler.register(ShutdownTask {
            count: Arc::clone(&run_count),
        });
        scheduler.start();
...
mod optional {
    ...
    fn serialize_struct_with_optional_fields() {
        // PRD-mandated verification for JOLTR-RS-163: Some → inner value,
