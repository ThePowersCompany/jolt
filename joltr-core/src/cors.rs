//! CORS preflight short-circuit `tower::Layer` (JOLTR-RS-056).
//!
//! [`CorsLayer`] wraps an inner [`tower::Service`] (typically JoltR's
...
//! non-empty (JOLTR-RS-057). Existing values on the response are not overwritten
//! — if the inner service has already set either header, the layer leaves it
//! alone.
//!
//! Allow-Origin selection (JOLTR-RS-058) follows the standard CORS matching
...
//! JOLTR-RS-055: a config whose `allow_origins`/`allow_methods`/`allow_headers`
    ...
    S::Error: Send + 'static,
{
    ...
    fn call(&mut self, mut req: AxumRequest) -> Self::Future {
        ...
        // Mirror Router's preserve-or-inject contract from JOLTR-RS-035 so the
...
/// [`CorsConfig`] (JOLTR-RS-058). Shared by both the preflight short-circuit
...
/// onto a non-preflight response per JOLTR-RS-057. Mutates the response in
...
/// JOLTR-RS-056). If the inner service has already set either header, the
...
/// so the matching rule lives in one place (JOLTR-RS-058).
fn inject_response_cors_headers(
    ...
    // Empty config → no header (preserves the JOLTR-RS-055 restrictive-default
...
/// CORS" contract pinned by JOLTR-RS-055.
///
/// `allowed_origin` is precomputed by [`select_allowed_origin`] and passed in
/// so the matching rule lives in one place (JOLTR-RS-058).
