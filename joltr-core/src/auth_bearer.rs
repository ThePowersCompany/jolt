//! Bearer-token authentication `tower::Layer` (JOLTR-RS-071..074).
//!
//! [`AuthBearerLayer`] (JOLTR-RS-071) extracts the `Authorization` request
...
//! downstream consumer (the upcoming JOLTR-RS-072 JWT validator) can pull it
//! back out.
//!
...
//!
//! Architectural decisions pinned here for JOLTR-RS-072..074 to build on:
//!
...
//!    decoding/expiry/signature checks land in JOLTR-RS-072; claims extraction
//!    into request extensions lands in JOLTR-RS-073. Splitting the slices
...
/// successful format validation; consumed by JOLTR-RS-072's JWT validator and
/// JOLTR-RS-073's claims extractor.
///
...
/// (JOLTR-RS-071). See module docs for the architectural contract (scope,
    ...
    S::Error: Send + 'static,
{
    ...
    fn call(&mut self, mut req: AxumRequest) -> Self::Future {
        ...
        // `ParseBodyService::call` (JOLTR-RS-059) and `CorsService::call`
        // (JOLTR-RS-056).
