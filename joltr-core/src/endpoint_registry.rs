//! Storage layer for HTTP endpoints registered with a JoltR server.
//!
//! [`EndpointRegistry`] owns reference-counted [`Endpoint`] trait objects and
//! is the collection that backs [`crate::JoltRServer::endpoint`] (JOLTR-RS-026).
//! JOLTR-RS-029 landed the struct + `register`, JOLTR-RS-030 added `sort` +
//! `iter`, and JOLTR-RS-031 added [`Self::build_router`] which converts the
...
impl EndpointRegistry {
    ...
    /// endpoint is wrapped in a closure that builds a JoltR [`Request`] from
...
/// Convert an inbound [`AxumRequest`] into a JoltR [`Request`] snapshot. Used by
/// [`EndpointRegistry::build_router`]'s per-route closures and by
/// [`crate::router::Router`]'s registry-driven dispatch path (JOLTR-RS-034). The
