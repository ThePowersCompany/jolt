//! `jolt-db` CLI binary (JOLT-RS-104).
//!
//! Currently exposes one subcommand: `jolt-db migrate new <name>` —
//! scaffolds a new empty migration file at
//! `migrations/<YYYYMMDDHHMMSS>_<name>.sql`. The actual file creation
//! lives in the library as [`jolt_db::create_migration_file`] (see lib
...
/// `jolt-db` — Postgres migration tooling for the Jolt framework.
#[derive(Parser)]
#[command(name = "jolt-db", version, about, long_about = None)]
struct Cli {
    #[command(subcommand)]
    command: Command,
}
    ...
    match cli.command {
        Command::Migrate { action } => match action {
            MigrateAction::New { name } => {
                ...
                match jolt_db::create_migration_file("migrations", &name, now) {
                    Ok(path) => {
