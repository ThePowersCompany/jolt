//! JWT signing/verification helpers (JOLTR-RS-072).
//!
//! Two-half surface:
//!
...
//!    [`AuthJwtLayer`](../../../joltr_core/auth_jwt/struct.AuthJwtLayer.html)
...
//!    (JOLTR-RS-148).** Any JWT payload key that isn't one of the explicit
...
/// fine-grained control over validation flags. JOLTR-RS-150.
...
/// JWT string (`header.payload.signature`). JOLTR-RS-149.
///
...
/// stripped — JOLTR-RS-071's [`AuthBearerLayer`] handles that stripping).
///
...
mod tests {
    ...
    fn decode_valid_hs256_token_returns_claims() {
        let secret = b"joltr-rs-072-test-secret";
    ...
    fn decode_token_with_custom_claims_surfaces_them_in_custom() {
        ...
        let secret = b"joltr-rs-074-custom-claims-secret";
    ...
    fn encode_produces_three_segment_jwt_string() {
        ...
        let secret = b"joltr-rs-149-encode-test";
        let claims = JwtClaims {
    ...
    fn encode_decode_simple_round_trip_preserves_claims() {
        ...
        let secret = b"joltr-rs-150-round-trip-secret";
        let claims = JwtClaims {
    ...
    fn decode_simple_rejects_expired_token() {
        ...
        let secret = b"joltr-rs-150-expired";
        let claims = JwtClaims {
