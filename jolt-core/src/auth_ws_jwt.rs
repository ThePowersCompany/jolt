//! WebSocket JWT validation `tower::Layer` (JOLT-RS-076..077).
//!
//! [`AuthWsJwtLayer`] (JOLT-RS-076) composes
//! [`extract_jwt_token`](crate::auth_websocket::extract_jwt_token) (JOLT-RS-075)
//! with [`jolt_utils::jwt::decode`] (JOLT-RS-072). It runs as a pre-upgrade
//! gate on a WebSocket-bearing route: on a valid `Sec-WebSocket-Protocol:
//! jolt-jwt, <token>` header carrying a token that decodes against the
...
//! (which performs the actual WebSocket upgrade in JOLT-RS-077). On any
...
//!
//! Architectural decisions pinned here for JOLT-RS-077 to build on:
//!
...
//!    pure helper (JOLT-RS-075) and 076 composes it with the decode in a
...
//!    jolt-jwt, <token>` shape is JWT-specific by construction, so there is
...
//!    for emitting `Sec-WebSocket-Protocol: jolt-jwt` on the upgrade
...
/// `Sec-WebSocket-Protocol: jolt-jwt, <token>` header BEFORE the upgrade is
/// allowed (JOLT-RS-076). On success, stashes both a [`WsJwtToken`] and a
/// [`JwtClaims`] into request extensions and delegates to the inner service
    ...
    S::Error: Send + 'static,
{
    ...
    fn call(&mut self, mut req: AxumRequest) -> Self::Future {
        ...
        // `AuthJwtService::call` (JOLT-RS-072).
