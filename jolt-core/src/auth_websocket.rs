//! WebSocket subprotocol JWT-token extraction (JOLT-RS-075..077).
//!
//! [`extract_jwt_token`] (JOLT-RS-075) parses an inbound
//! `Sec-WebSocket-Protocol` header value carrying the canonical
//! `jolt-jwt, <token>` shape and returns the bare JWT string. The validator
//! tower::Layer that consumes this helper before allowing a WebSocket upgrade
//! lands in JOLT-RS-076; the JwtClaims-into-handler injection lands in
//! JOLT-RS-077.
//!
//! Architectural decisions pinned here for JOLT-RS-076..077 to build on:
//!
//! 1. **Layer scope is ONLY format extraction.** "Header is present, parses as
//!    `jolt-jwt, <token>`, and yields a non-empty token" is the entire 075
//!    contract. JWT decoding/expiry/signature checks land in JOLT-RS-076 (which
//!    will compose this helper with [`jolt_utils::jwt::decode`] inside a
//!    `tower::Layer` that runs BEFORE the WS upgrade); claims injection into
//!    the WS handler lands in JOLT-RS-077. Splitting the slices keeps each
...
//!
//! 2. **Canonical "jolt-jwt, <token>" shape: exactly two comma-separated
//!    subprotocols.** Browser clients calling
//!    `new WebSocket(url, ["jolt-jwt", "<token>"])` emit
//!    `Sec-WebSocket-Protocol: jolt-jwt, eyJ...` — exactly two values. The
...
//!
//! 3. **`jolt-jwt` literal match is case-sensitive.** Per RFC 6455 §11.5,
...
//!    string compare). A header of `JOLT-JWT, eyJ...` is rejected with
//!    [`WsTokenRejectReason::MissingJoltJwtPrefix`] — distinct from the
...
//!    around list element separators. A header of `jolt-jwt,eyJ...` (no
//!    space) and `jolt-jwt , eyJ...` (extra space) both yield the same
...
//!    [`WsTokenRejectReason::EmptyToken`].** A header of `jolt-jwt,` (or
//!    `jolt-jwt, ` with trailing whitespace) is structurally parseable but
...
//!    076 only adds the wiring; mirrors the JOLT-RS-071 [`BearerToken`] /
//!    JOLT-RS-072 [`AuthJwtLayer`] split. The newtype gives the layer a
...
/// `new WebSocket(url, ["jolt-jwt", "<token>"])` produce a header of the
/// canonical form `jolt-jwt, <token>` that this module extracts.
///
/// Held as a `pub const` (rather than inlined as a string literal in
/// [`extract_jwt_token`]) so JOLT-RS-076's tower::Layer and the test module
/// can refer to the marker by name; a future rename of the marker is
/// localized to this constant.
pub const JOLT_JWT_SUBPROTOCOL: &str = "jolt-jwt";

/// JWT token extracted from a `Sec-WebSocket-Protocol: jolt-jwt, <token>`
...
///
/// JOLT-RS-075 only defines the type — the future JOLT-RS-076 layer is the
...
/// can produce a distinct 401 body once JOLT-RS-076 wires this helper into
...
pub enum WsTokenRejectReason {
    ...
    /// literal [`JOLT_JWT_SUBPROTOCOL`]. Both shapes are rejected with this
    /// variant since both indicate the canonical `jolt-jwt, <token>` shape
    /// is absent.
    MissingJoltJwtPrefix,
...
impl WsTokenRejectReason {
    /// Static `text/plain` body emitted in the future JOLT-RS-076 401
...
/// Validate that `header` carries a `Sec-WebSocket-Protocol: jolt-jwt, <token>`
/// value and return the extracted token. Whitespace around the comma is
/// tolerated (per RFC 7230 §7 BWS); the `jolt-jwt` literal match is
/// case-sensitive (per RFC 6455 §11.5). See module docs for the full
/// architectural contract.
///
/// JOLT-RS-075 contract: this helper is the entire surface. JOLT-RS-076's
/// tower::Layer will compose this with [`jolt_utils::jwt::decode`] before
...
pub fn extract_jwt_token(header: Option<&HeaderValue>) -> Result<String, WsTokenRejectReason> {
    ...
    let marker = parts
        .next()
        .ok_or(WsTokenRejectReason::MissingJoltJwtPrefix)?
        .trim();
    if marker != JOLT_JWT_SUBPROTOCOL {
        return Err(WsTokenRejectReason::MissingJoltJwtPrefix);
    }
