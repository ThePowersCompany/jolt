...
//!
//! Phase08 ladder:
//! - JOLTR-RS-038: parse the path string literal from the attribute tokens. (landed)
//! - JOLTR-RS-039: scan the impl block for `#[get]`/`#[post]`/`#[put]`/
//!   `#[patch]`/`#[delete]` methods, collect their signatures. (landed)
//! - JOLTR-RS-040: validate handler signatures — first arg is `&self`, return
//!   type is `Response<T>` or `Result<Response<T>, E>`. (landed)
//! - JOLTR-RS-041: emit the (Method, path) -> handler match. (landed)
//!
//! Phase09 ladder:
//! - JOLTR-RS-042: wire the parsing + codegen pipeline into the proc-macro
//!   entry point and emit one `inventory::submit!` block per discovered
//!   method, registering a [`joltr_core::RegisteredEndpoint`] so
//!   `JoltRServer::start` (JOLTR-RS-044) can collect them via
//!   `inventory::iter::<RegisteredEndpoint>()`. (landed)
//! - JOLTR-RS-043 (this iteration): emit one `__jolt_handler_<name>` axum-
//!   compatible async wrapper per discovered method via
//!   [`generate_handler_wrappers`]. Each wrapper takes a `::joltr_core::Request`,
...
//!   [`IntoResponse`] trait. JOLTR-RS-044 will extend
...
//! their own proc-macro attributes. JOLTR-RS-042 wires [`strip_verb_attrs`] into
...
#[allow(dead_code)] // `path` is read by tests this iteration; JOLTR-RS-041 wires it into codegen.
...
/// `joltr_core::Method` but lives in the macro crate to avoid a cyclic dep
/// (joltr-core depends on joltr-macros at the workspace level once macros are
/// re-exported). Codegen via [`generate_dispatch_match`] emits
/// `::joltr_core::Method::<variant>` paths using
/// [`HttpMethod::as_joltr_core_variant_ident`].
...
impl HttpMethod {
    ...
    /// `options`) which are handled by joltr-core's auto-OPTIONS / Allow-header
    ...
    /// Identifier for the matching variant in `::joltr_core::Method`, suitable
    /// for splicing into a `quote!` path like `::joltr_core::Method::#ident`.
    /// The five user-routable verbs map 1:1 onto joltr-core's enum variants
    /// (`Method::Get`, `Method::Post`, ...); HEAD/OPTIONS live on joltr-core's
    /// enum but are framework-handled, so this enum has no variants for them.
    fn as_joltr_core_variant_ident(self) -> Ident {
        match self {
...
/// without holding a borrow on the input `ItemImpl`. JOLTR-RS-040 will read
...
///    prefixes — a user-typed `crate::Response<T>`, `joltr_core::Response<T>`,
...
/// Emit `::joltr_core::inventory::submit! { ... }` blocks — one per discovered
/// method — that register a [`::joltr_core::RegisteredEndpoint`] for the
/// (path, method, handler) triple. [`crate::expand_endpoint`] arranges for
/// `JoltRServer::start` (JOLTR-RS-044) to call
/// `inventory::iter::<RegisteredEndpoint>()` and register every entry across
/// all linked crates into an [`::joltr_core::EndpointRegistry`].
///
...
fn generate_inventory_submits(
    ...
    let submits = methods.iter().map(|m| {
        let variant = m.http_method.as_joltr_core_variant_ident();
        let user_fn = &m.sig.ident;
        let wrapper_name = format_ident!("__jolt_handler_{}", user_fn);
        quote! {
            ::joltr_core::inventory::submit! {
                ::joltr_core::RegisteredEndpoint {
                    path: #path,
                    method: ::joltr_core::Method::#variant,
...
/// endpoint type. Each wrapper takes a `::joltr_core::Request`, constructs a
...
/// ```ignore
/// #[doc(hidden)]
/// pub fn __jolt_handler_<name>(__req: ::joltr_core::Request) -> ::joltr_core::EndpointFuture {
///     ::std::boxed::Box::pin(async move {
///         let _ = __req; // consumed by JOLTR-RS-046+ auto-middleware extraction
...
///    for JOLTR-RS-042 flagged the construct-`Self` problem as non-trivial.
...
/// 3. **`__req` is bound but unused.** Until JOLTR-RS-046 (auto-middleware)
...
///    implements `IntoResponse` via the impl in `joltr-core/src/response.rs`
...
/// should call `MyEndpoint::__jolt_handler_xxx` directly. JOLTR-RS-044 will
...
/// the user gets a single targeted diagnostic — the same shape JOLTR-RS-038
...
/// keyed on `(::joltr_core::Method, path)` and resolving to the handler's
...
///     (::joltr_core::Method::Get,  "/api/test") => Self::list,
///     (::joltr_core::Method::Post, "/api/test") => Self::create,
///     _ => unreachable!("..."),
/// }
/// ```
///
...
///
/// JOLTR-RS-042 will wire this into the proc-macro entry point alongside
/// inventory-style registration; JOLTR-RS-043 will adapt the
/// `Self::<handler>` references into tower-compatible async fns.
#[allow(dead_code)] // tests consume it; lib.rs wires it in once JOLTR-RS-042/043 land.
pub(crate) fn generate_dispatch_match(
    ...
    let arms = methods.iter().map(|m| {
        ...
        quote! {
            (::joltr_core::Method::#variant, #path) => Self::#fn_ident,
        }
    });
    quote! {
        match (__method, __path) {
...
mod tests {
    ...
    // ----- JOLTR-RS-040: validate_signature / validate_methods -----
    ...
    // ----- JOLTR-RS-041: generate_dispatch_match -----
    ...
    #[test]
    fn arms_reference_joltr_core_method_variants_and_handler_fn_names() {
        // Pin the arm-body shape: each arm pattern names the right
        // ::joltr_core::Method variant and path literal, and resolves to the
        ...
        assert!(
            rendered.contains(":: joltr_core :: Method :: Get"),
            "missing Method::Get variant; rendered: {rendered}"
        );
        assert!(
            rendered.contains(":: joltr_core :: Method :: Delete"),
    ...
    // ----- JOLTR-RS-042: strip_verb_attrs / generate_inventory_submits / expand_endpoint -----
    ...
    fn generate_inventory_submits_one_per_method() {
        ...
        // independent inventory::submit! block, so JOLTR-RS-044's iter() will
        ...
        for variant in ["Get", "Post", "Delete"] {
            let needle = format!(":: joltr_core :: Method :: {variant}");
            assert!(
    ...
    fn expand_endpoint_emits_impl_plus_submit_for_single_method() {
        ...
        // joltr-core/tests/inventory_registration.rs runs the same shape end-
        ...
        // Submit emitted.
        assert!(
            rendered.contains(":: joltr_core :: inventory :: submit"),
    ...
    // ----- JOLTR-RS-043: generate_handler_wrappers -----
    ...
    fn generate_handler_wrappers_emits_one_fn_per_method() {
        ...
        // JOLTR-RS-044's inventory record can reference them via
        ...
        let req_count = rendered.matches(":: joltr_core :: Request").count();
        assert_eq!(
            req_count, 2,
            "each wrapper takes one Request, so the path must appear twice; rendered: {rendered}"
        );
        let fut_count = rendered.matches(":: joltr_core :: EndpointFuture").count();
        assert_eq!(
            fut_count, 2,
    ...
    fn expand_endpoint_emits_handler_wrapper_alongside_submits() {
        ...
        // emitted. The integration test in joltr-core/tests/inventory_registration.rs
