...
pub const DateTime = struct {
    ...
    /// Memory is owned by the caller and must be freed.
    pub fn toString(self: DateTime, alloc: std.mem.Allocator) ![:0]u8 {
        const buf = try alloc.allocSentinel(u8, 27, 0);
        _ = self.bufWrite(buf);
        return buf;
    }
...
/// Convert an 8601 date 'YYYY-MM-DD' to Unix time in microseconds
pub fn dateToUnix(date: []const u8) !i64 {
    const parsed = try Date.parse(date, .iso8601);
    const date_time = try DateTime.initUTC(parsed.year, parsed.month, parsed.day, 0, 0, 0, 0);
    return date_time.unix(.microseconds);
}

/// Calculate the difference in days between start and end dates
/// start_date an 8601 date 'YYYY-MM-DD'
/// end_date an 8601 date 'YYYY-MM-DD'
pub fn diffInDays(start_date: []const u8, end_date: []const u8) !i32 {
    const MICROS_PER_DAY = 86_400_000_000;

    const start_date_unix = try dateToUnix(start_date);
    const end_date_unix = try dateToUnix(end_date);

    return @intCast(@divTrunc(end_date_unix - start_date_unix, MICROS_PER_DAY));
}
