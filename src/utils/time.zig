// From https://github.com/nektro/zig-time
...
pub const Format = struct {
    ...
    pub const ISO = "YYYY-MM-DDTHH:mm:ss.SSSZZZ";
...
/// Allocates and formats milliseconds to a string.
/// Caller owns the returned memory.
pub fn formatMs(alloc: std.mem.Allocator, milliseconds: anytype, comptime fmt: string) ![]const u8 {
    comptime std.debug.assert(@typeInfo(@TypeOf(milliseconds)) == .int);
    // TODO: Another possible comptime solution, but might make the API annoying?
    // comptime std.debug.assert(milliseconds.signedness == .unsigned);
    if (milliseconds < 0) {
        std.log.err("milliseconds provided: {any}", .{milliseconds});
        return error.TimeMustBePositive;
    }
    return try time.DateTime.initUnixMs(@intCast(milliseconds)).formatAlloc(alloc, fmt);
}

test {
    const alloc = std.testing.allocator;

    const t = time.DateTime.initUnix(1330502962);
    const actual = try t.formatAlloc(alloc, Format.ISO);
    defer alloc.free(actual);

    try std.testing.expectEqualStrings(actual, "2012-02-29T08:09:22.000Z");
}
